...
Form::Form(ProjectSettings* settings, QWidget* parent) : QWidget(parent), projectSettings(settings) {
    ...
    channelComboBox->addItem(Common::Constants::Channel::Alpha);
...
void Form::populateUpdate(const ListModel::Update& update) {
    ...
    sizeWindowsLabel->setText(update.size.contains(Common::Constants::OS::Windows) ? QString::number(update.size[Common::Constants::OS::Windows]) : "");
    sizeLinuxLabel->setText(update.size.contains(Common::Constants::OS::Linux) ? QString::number(update.size[Common::Constants::OS::Linux]) : "");
    sizeMacOSLabel->setText(update.size.contains(Common::Constants::OS::MacOS) ? QString::number(update.size[Common::Constants::OS::MacOS]) : "");
...
ListModel::Update Form::update() const {
    ...
    if (!sizeWindowsLabel->text().isEmpty()) {
        update.size[Common::Constants::OS::Windows] = sizeWindowsLabel->text().toInt();
    }

    if (!sizeLinuxLabel->text().isEmpty()) {
        update.size[Common::Constants::OS::Linux] = sizeLinuxLabel->text().toInt();
    }

    if (!sizeMacOSLabel->text().isEmpty()) {
        update.size[Common::Constants::OS::MacOS] = sizeMacOSLabel->text().toInt();
    }
...
void Form::build() {
    ...
    QString zipDir = projectSettings->projectDir() + "/" + Common::Constants::CurrentOS;
    QDir dir;
    dir.mkpath(zipDir);

    zipPath = zipDir + "/" + updateDirName + ".zip";

    Common::ZipCompressor::compress(zipPath, updateDirPath + "/");

    QFileInfo fi(zipPath);
    setFileSize(Common::Constants::CurrentOS, fi.size());
...
void Form::setFileSize(const QString& os, qint64 size) {
    ...
    if (os == Common::Constants::OS::Windows) {
        sizeWindowsLabel->setText(value);
    } else if (os == Common::Constants::OS::Linux) {
        sizeLinuxLabel->setText(value);
    } else if (os == Common::Constants::OS::MacOS) {
